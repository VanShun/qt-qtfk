...
void LoadingProgress::setMaxDiameter(float max)
{
    m_maxDiameter = max;

    const float half = qMin(width(), height()) / 2;    //大圆轨迹半径
...
void LoadingProgress::setMinDiameter(float min)
{
    m_minDiameter = min;
    update();
}
...
void LoadingProgress::resizeEvent(QResizeEvent *event)
{
    ...
    const float half = qMin(width(), height()) / 2;    //大圆轨迹半径
    const float centerDistance = half - m_maxDiameter / 2 - 1;  //小圆圈圆心到大圆圆心距离

    const float gap = (m_maxDiameter - m_minDiameter) / (m_count - 1) / 2; //小圆半径递增增幅
    const float angleGap = (float)360 / m_count;    //小圆圆心跟x轴夹角角度递增增幅
